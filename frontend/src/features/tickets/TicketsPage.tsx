      ...
      <article className="lg:col-span-5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-card p-6 shadow-sm flex flex-col h-full">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Daftar laporan kendala pelanggan.</p>
          </div>
          <div className="flex items-center gap-2">
            <select
              className="bg-white dark:bg-slate-900 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              value={statusFilter}
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden border-r border-slate-100 dark:border-slate-800">
              ...
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start bg-slate-50/20 dark:bg-slate-950/10">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <h3 className="font-bold text-slate-900 dark:text-slate-50 dark:text-white text-sm">#{detail.id} - {detail.nama}</h3>
                    <StatusPill
                  ...
                  <div className="flex flex-col gap-1 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <div className="mt-2.5 p-3 bg-slate-50 dark:bg-slate-950 dark:bg-slate-950/50 rounded-xl text-slate-700 dark:text-slate-300 border-l-2 border-indigo-400 text-xs leading-relaxed shadow-sm">
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30 dark:bg-slate-950/5 flex flex-col gap-4 scrollbar-thin">
                {detail.messages.length === 0 ? (
                  <div className="text-center py-8 text-slate-400 dark:text-slate-500 text-xs">Belum ada tanggapan percakapan.</div>
                ) : (
                  detail.messages.map((m) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 mt-1 px-1 text-[9px] text-slate-400 dark:text-slate-500 select-none">
                          <span>{time}</span>
                          {isAdmin && (
                ...
                {detail.status === "closed" ? (
                  ...
                  <form onSubmit={handleSendReply} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-slate-50 dark:bg-slate-950 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 dark:border-slate-700 rounded-xl px-4 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 dark:text-white transition-all"
                      value={replyText}
                    ...
                    {replyText.trim() && (
                      <div className="inline-flex items-center">
                        ...
                        <Button
                          ...
                          className="!py-2 !px-3 !rounded-l-none !rounded-r-xl border border-l-0 border-slate-200 dark:border-slate-800 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700"
            ...
            {detail.pelanggan_id && (
              <div className="w-full lg:w-72 bg-slate-50/50 dark:bg-slate-950/30 flex flex-col h-full overflow-y-auto p-4 gap-4 border-t lg:border-t-0 border-slate-100 dark:border-slate-800 scrollbar-thin">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 shadow-sm">
                  ...
                  {loadingCust ? (
                    <div className="py-6 text-center text-xs text-slate-400 dark:text-slate-500 flex items-center justify-center gap-1.5">
                    ...
                    <div className="text-xs text-slate-400 dark:text-slate-500 py-2">Gagal memuat profil pelanggan.</div>
                  )}
                </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 shadow-sm flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-2.5">
                      <div className="flex justify-between text-xs items-center">
                        <span className="text-slate-400 dark:text-slate-500">SN ONT</span>
                        <code className="font-semibold font-mono text-slate-800 dark:text-slate-100 dark:text-slate-200">{linkedCustomer.sn_ont}</code>
                      </div>
                      ...
                      {ontStatus ? (
                        <div className="space-y-2 mt-1.5 text-xs border-t border-slate-50 dark:border-slate-800 pt-2.5 animate-in fade-in duration-200">
                          <div className="flex justify-between items-center">
                            <span className="text-slate-400 dark:text-slate-500">Koneksi</span>
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 text-center py-3 italic border-t border-slate-50 dark:border-slate-800 pt-3">
            ...
            <form onSubmit={handleCreateTicketSubmit} className="p-5 flex flex-col gap-3.5">
              ...
              <label className="flex flex-col gap-1">
                ...
                <textarea
                  required
                  rows={3}
                  className="w-full text-xs px-3 py-2 border rounded-xl bg-white dark:bg-slate-900 dark:bg-slate-800 text-slate-800 dark:text-slate-100 dark:text-slate-300 focus:ring-2 focus:ring-indigo-500"
