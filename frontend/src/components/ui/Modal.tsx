...
export function Modal({ title, children, actions, onClose }: ModalShellProps) {
  return (
    <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md z-50 flex items-center justify-center p-4" role="presentation" onClick={onClose}>
      <section
        className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden animate-in"
        role="dialog"
        aria-modal="true"
        aria-label={title}
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-slate-800">
          ...
          <button type="button" className="text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-slate-800 dark:hover:text-slate-300 p-2 rounded-lg transition-colors cursor-pointer" onClick={onClose} aria-label="Tutup dialog">
            Tutup
          </button>
        </div>
