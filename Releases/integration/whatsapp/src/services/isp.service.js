...
const notifyAdminViaDiscord = async (info, customMsg = null) => {
    ...
    const cleanPhone = info.phone.replace(/@c\.us$/, '').replace(/^\+/, '');
    const linkNumber = cleanPhone.startsWith('62') ? cleanPhone : '62' + cleanPhone.replace(/^0/, '');
    const mention = process.env.DISCORD_ADMIN_ROLE_ID ? `<@&${process.env.DISCORD_ADMIN_ROLE_ID}>` : '<@989203245895401533>, <@923237802618007562>, <@173778121059860480>';
...
const createPaymentConfirmation = async (tagihanId, pelangganId, buktiTransfer, catatan, linkedTagihanIds = '') => {
    try {
        const res = await client.post('/api/v1/chatbot/confirmations', {
            ...
            catatan,
            linked_tagihan_ids: linkedTagihanIds
        });
        return res.data;
    } catch (err) {
