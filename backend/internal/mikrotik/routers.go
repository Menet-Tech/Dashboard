...
func (s *RouterService) List(ctx context.Context) ([]Router, error) {
	rows, err := s.DB.QueryContext(ctx, `
		SELECT id, nama, host, username, password, is_active, created_at, updated_at
	...
	for rows.Next() {
		var r Router
		var isActive int
		if err := rows.Scan(&r.ID, &r.Name, &r.Host, &r.Username, &r.Password, &isActive, &r.CreatedAt, &r.UpdatedAt); err != nil {
...
func (s *RouterService) FindByID(ctx context.Context, id int64) (Router, error) {
	...
	var r Router
	var isActive int
	if err := row.Scan(&r.ID, &r.Name, &r.Host, &r.Username, &r.Password, &isActive, &r.CreatedAt, &r.UpdatedAt); err != nil {
...
func (s *RouterService) Create(ctx context.Context, r Router) (Router, error) {
	...
	result, err := s.DB.ExecContext(ctx, `
		INSERT INTO mikrotik_routers (nama, host, username, password, is_active, updated_at)
		VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
	`, r.Name, r.Host, r.Username, r.Password, isActive)
	if err != nil {
...
func (s *RouterService) Update(ctx context.Context, id int64, r Router) (Router, error) {
	r.Name = strings.TrimSpace(r.Name)
	r.Host = strings.TrimSpace(r.Host)
	r.Username = strings.TrimSpace(r.Username)
	r.Password = strings.TrimSpace(r.Password)
	...
	if r.Password != "" {
		result, err = s.DB.ExecContext(ctx, `
			...
			SET nama = ?, host = ?, username = ?, password = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP
			WHERE id = ?
		`, r.Name, r.Host, r.Username, r.Password, isActive, id)
	} else {
		result, err = s.DB.ExecContext(ctx, `
			UPDATE mikrotik_routers
			SET nama = ?, host = ?, username = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP
			WHERE id = ?
		`, r.Name, r.Host, r.Username, isActive, id)
	}
