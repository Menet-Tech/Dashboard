...
class Pelanggan extends BaseModel
{
    ...
    public function getJatuhTempo(int $limitAfterDays = 5): array
    {
        ...
        return array_values(array_filter(array_map([$this, 'hydrateDerivedDates'], $rows), function (array $row) use ($limitAfterDays): bool {
            return $this->daysPastDueDate($row['due_date']) >= $limitAfterDays;
        }));
    }

    public function getReminderCustomers(int $daysBeforeDue = 3): array
    {
        ...
        return array_values(array_filter(array_map([$this, 'hydrateDerivedDates'], $rows), function (array $row) use ($daysBeforeDue): bool {
            return $this->daysUntilDueDate($row['due_date']) === $daysBeforeDue;
        }));
    }
    ...
    private function daysPastDueDate(string $dueDate): int
    {
        return max(0, -1 * $this->daysUntilDueDate($dueDate));
    }
}
