        ...
        <table class="table table-hover align-middle">
            ...
            <?php foreach ($rows as $row): ?>
                <tr>
                    ...
                    <td>
                        <form method="post" action="<?= base_url('/pelanggan/status') ?>" class="d-flex gap-2 align-items-center">
                            <?= csrf_field() ?>
                            <input type="hidden" name="id" value="<?= (int) $row['id'] ?>">
                            <select name="status" class="form-select form-select-sm">
                                <?php foreach (['active' => 'Active', 'limit' => 'Limit', 'inactive' => 'Inactive'] as $value => $label): ?>
