...
class Scheduler
{
    ...
    public function run(): void
    {
        ...
        $this->processAutoBackup();
        $this->processMenungguWa();
        $this->processJatuhTempo();
        $this->processReminderBeforeDue();
    ...
    public function processJatuhTempo(): void
    {
        ...
        $limitAfterDays = max(1, (int) Pengaturan::get('billing_limit_after_days', '5'));
        foreach ($pelangganModel->getJatuhTempo($limitAfterDays) as $customer) {
            ...
            if ($limited) {
                $template = TemplateWA::getByTrigger('limit_5hari') ?? TemplateWA::getByTrigger('jatuh_tempo');
                ...
                discordNotify(
                    ...
                    "Pelanggan {$customer['nama']} melewati {$limitAfterDays} hari setelah jatuh tempo dan dilimit oleh scheduler.",
                    [
    ...
    public function processReminderBeforeDue(): void
    {
        $pelangganModel = new Pelanggan();
        $daysBeforeDue = max(1, (int) Pengaturan::get('billing_reminder_days_before', '3'));
        foreach ($pelangganModel->getReminderCustomers($daysBeforeDue) as $customer) {
            $template = TemplateWA::getByTrigger('reminder_custom') ?? TemplateWA::getByTrigger('reminder_7hari');
            $message = TemplateWA::parse($template['isi_pesan'] ?? 'Reminder tagihan.', $customer);
            $result = $this->whatsApp->sendText((string) $customer['no_wa'], $message);
            ActionLog::create((int) $customer['id'], 'WA_SENT', $result['success'] ? 'success' : 'failed', $result['error'] ?? 'Reminder sebelum jatuh tempo');
            if (!$result['success']) {
                discordNotify(
                    'Reminder WA Gagal',
                    "Reminder {$daysBeforeDue} hari gagal dikirim untuk {$customer['nama']}.",
                    [['name' => 'Error', 'value' => $result['error'] ?? 'Unknown error', 'inline' => false]],
                    'alert',
                    'danger',
                    'wa_failed'
    ...
    public function processAutoBackup(): void
    {
        if (!DatabaseBackup::shouldRunAutoBackup()) {
            return;
        }

        $result = DatabaseBackup::create();
        DatabaseBackup::markAutoBackupRunToday();
        ActionLog::create(null, 'AUTO_BACKUP', $result['success'] ? 'success' : 'failed', $result['message']);
    }
}
