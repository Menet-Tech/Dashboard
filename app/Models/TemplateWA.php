...
class TemplateWA extends BaseModel
{
    ...
    {
        $instance = new self();
        $stmt = $instance->db->prepare('SELECT * FROM template_wa WHERE trigger_event = :trigger LIMIT 1');
    ...
    {
        $stmt = $this->db->prepare('UPDATE template_wa SET nama = :nama, isi_pesan = :isi_pesan, is_active = :is_active WHERE id = :id');
        foreach ($rows as $row) {
            $stmt->execute($row);
        }
    }
